...
class TabBar extends Component {
	...
	uploadImage(image) {
		...
		const filename = `${Config.AWS_BOOKS_PICTURES_FOLDER_NAME}${uuidv1()}.${image.fileName
			.split('.')
			.pop()}`;
	...
	render() {
		...
		return (
			<View
				style={[styles.tabBar, styles.tabBarCamera]}
				ref={ref => {
					this.tabBar = ref;
				}}
			>
				<TouchableOpacity onPress={handleClose} style={styles.tab}>
					{/* <View style={[styles.cameraTabInner]}> */}
					<Icon name="undo" size={20} style={styles.image} />
					{/* </View> */}
				...
				>
					<View style={[styles.cameraTabInner, styles.cameraTabInnerActive]}>
						...
						<Icon name="camera" size={30} style={[styles.image, styles.cameraImage]} />
					</View>
				</TouchableOpacity>

				<TouchableOpacity onPress={this.handleChooseFromAlbum.bind(this)} style={styles.tab}>
					{/* <Image source={Images.cameraPhotoLibrary} style={styles.image} /> */}
					<Icon name="images" size={20} style={styles.image} />
				</TouchableOpacity>
			</View>
		);
	}
